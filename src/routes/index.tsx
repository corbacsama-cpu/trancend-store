...
function HeroCarousel(props: { slides: HeroSlide[] }) {
  ...
  const slides = () =>
    props.slides.length > 0 ? props.slides : MOCK_HERO_SLIDES;
    ...
    <section class="hero">
      ...
      <div class="container hero-content" style="position:relative;z-index:4">
        <p class="hero-eyebrow">
          {slides()[current()]?.title || "Collection 2025"}
        </p>
        <h1 class="hero-title">
          TRÄN
          <br />
      ...
      <div style="position:absolute;bottom:32px;right:32px;z-index:5;display:flex;align-items:center;gap:16px">
        ...
        <div style="display:flex;gap:8px;align-items:center">
          <For each={slides()}>
            {(_, i) => (
              <button
                class={`hero-dot ${current() === i() ? "active" : ""}`}
                onClick={() => {
                  goTo(i());
                  restart();
                }}
      ...
      <div class="carousel-root">
        ...
        >
          <div
            ...
            style={`transform:translateX(-${index() * iw()}%);transition:transform 0.5s cubic-bezier(0.4,0,0.2,1);display:flex`}
          >
            <For each={items()}>
              {(p) => (
                <div
                  style={`flex:0 0 ${iw()}%;padding:0 8px;box-sizing:border-box`}
                >
                  <ProductCard
                    product={p}
                    onQuickAdd={(p) => {
        ...
        <div class="carousel-dots">
          <For each={Array.from({ length: total() })}>
            {(_, i) => (
              <button
                class={`carousel-dot ${index() === i() ? "active" : ""}`}
                onClick={() => {
                  clearInterval(timer);
                  setIndex(i());
                }}
 ...
 <section class="brand-statement">
        <div class="container brand-grid">
          <div class="brand-text">
            <p class="brand-eyebrow">TRÄNCËNÐ MOVEMENT</p>
            ...
            <p class="brand-description">
              ...
              dépassement. Nous créons des vêtements pour ceux qui veulent aller
              au-delà.
            </p>

            <A href="/about" class="brand-cta">
     ...
     {/* <section style="padding:80px 0;border-top:1px solid var(--gray-2);border-bottom:1px solid var(--gray-2);background:var(--gray-1)">
        <div class="container" style="text-align:center">
          <p style="font-family:var(--font-mono);font-size:11px;letter-spacing:0.3em;text-transform:uppercase;color:var(--accent);margin-bottom:16px">
            NOTRE PHILOSOPHIE
          </p>
          <h2 style="font-family:var(--font-display);font-size:clamp(48px,8vw,100px);letter-spacing:0.04em;line-height:1;color:var(--white);margin-bottom:24px">
            L'AMOUR DU
            <br />
            DÉPASSEMENT
          </h2>
          <p style="font-size:15px;color:var(--gray-4);max-width:560px;margin:0 auto 40px;line-height:1.8">
            Chaque pièce est conçue pour ceux qui cherchent à se surpasser.
            Qualité artisanale, design intemporel.
          </p>
          <A
            href="/about"
            style="font-family:var(--font-mono);font-size:11px;letter-spacing:0.2em;text-transform:uppercase;color:var(--accent);border-bottom:1px solid var(--accent);padding-bottom:4px"
          >
            EN SAVOIR PLUS →
          </A>
        </div>
